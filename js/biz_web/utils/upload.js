define("biz_web/utils/upload.js", ["widget/upload.css", "biz_web/lib/webuploader.js", "common/wx/dialog.js", "common/wx/Tips.js", "tpl/uploader.html.js"],
function(e) {
    "use strict";
    function i(e) {
    ...
    checkIsSupport = function(e) {
        var uploader = n.Uploader;
		if (0 == uploader.support("flash")) {
			p("<p>未安装或未正确配置flash插件，请检查后重试。<br><a href='http://get.adobe.com/cn/flashplayer/' target='_blank'>到adobe去下载flash插件</a></p>");
		} else if (0 == i.support()) {
			p("<p>您的浏览器不支持上传</p>")
		} else {
			e.refresh();
    ...
    h = function(e) {
        ...
        f = $(e.container);
        f.on("click", function() {
            Math.random() < .1 && log(12),
            checkIsSupport(t);
        }).parent().append(p),
        function() {
			// 上报
            if (0 == n.Uploader.support("html5") && 0 == n.Uploader.support("flash")) {
				(new Image).src = "/misc/jslog?level=error&id=36&content=[pageurl:" + encodeURIComponent(location.href) + ",ua:" + encodeURIComponent(window.navigator.userAgent) + "]";
			}
        } (),
        l = {
        ...
        s = c[e.type] || c[2],
        e = $.extend(true, {}, d, l, s, e);
        e.server;
        if (0 == n.Uploader.support("html5") && e.compress) {
			e.compress.quality = 70;
		}
        try {
            t = n.create(e);
        } catch(h) {
        ...
        t.on("beforeFileQueued", function(i) {
            ...
            return e.canContinueUpload && !e.canContinueUpload() ? false : !(e.onSelect && e.onSelect(null, i.id, i) === false);
        });
        t.on("fileQueued", function(file) {
            var i = {
                id: file.id,
                fileName: file.name,
                size: n.formatSize(file.size)
            };
            p.append(r(o, i)).show();
        });
        t.on("fileDequeued", function() {
            Math.random() < .1 && log(14),
            if (e.onCancel) {
				e.onCancel();
			}
        });
        t.on("uploadProgress", function(i, n) {
            ...
            a.width("%s%".sprintf(100 * n));
            1 == n && p.find(t).find(".js_cancel").remove();
            if (e.onProgress) {
				e.onProgress(null, i.id, i, {
					percentage: n
				});
			}
        });
        ...
        t.on("uploadSuccess", function(n, t, o) {
			Math.random() < .1 && log(16);
            if (t && t.base_resp) {
                var r = +t.base_resp.ret;
                if (0 == r) Math.random() < .1 && (log(17), g[n.id] && i( + new Date - g[n.id]));
                else switch (n.setStatus("invalid"), r) {
                case - 8:
				case 200018:
        ...
        t.on("uploadFinished", function(i) {
            ...
            p.fadeOut().html("");
            g = {};
            if (0 == i.numOfInvalid && i.numOfSuccess > 0 && e.onAllComplete) {
				e.onAllComplete(null, {
    ...
    return {
        ...
        uploadImageLibFile: function(e) {
            return e.url = s + "/cgi-bin/filetransfer?action=upload_material&f=json",
            "undefined" != typeof e.scene && (e.url += "&scene=" + e.scene),
            1 == e.doublewrite && (e.url += "&writetype=doublewrite&groupid=" + (e.groupid || 1)),
            h(e);
        },
